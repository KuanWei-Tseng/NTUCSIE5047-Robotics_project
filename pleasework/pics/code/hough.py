...
row, column = img.shape[0]/2, img.shape[1]
img2 = np.copy(img[row:img.shape[0], :])
cv2.imwrite("half.jpg", img2)
print("row = {}, column = {}".format(row, column))

# intensity equalization
hsl = cv2.cvtColor(img2, cv2.COLOR_BGR2HSV)
...
if y_offset >= 0:
    cv2.circle(img2,(int(y_offset),int(row)),7,(0,255,255),5)
if w_offset < column:
    cv2.circle(img2,(int(w_offset),int(row)),7,(255,255,255),5)

cv2.imwrite("road_lines.jpg",img2)
