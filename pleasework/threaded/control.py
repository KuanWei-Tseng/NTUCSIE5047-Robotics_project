class control:
    ...
    def findPIDval(dev):
        ...
        P = self.Kp * self.dev
        # I: Integral term:
        I = self.Ki * (self.int_dev + prev_dev)
        # D: Differential term:
        D = self.Kd * (dev - self.prev_dev)
        # Record current data:
        self.int_dev = self.int_dev + prev_dev
        self.prev_dev = dev
        # Calculate the PID control power
        PIDval = P + I + D
        return PIDval

    def PIDLaneFoller(PIDval,centSpd):
    ...
    def adaptive_control(rightSpd,leftSpd,dev,theta,type):
        ...
        if type == "b":
            ...
            dev = (-1)*abs(600-dev)* Kw  # Case: Turning Left 
